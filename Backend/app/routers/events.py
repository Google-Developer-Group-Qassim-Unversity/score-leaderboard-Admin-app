...
def mark_attendance(
    ...
    with SessionLocal() as session:
        try:
            ...
            if not event_log:
                write_log_exception(log_file, Exception(f"HTTP 500: Event [{event_id}] has no attendable logs!!"))
            ...
            form = form_queries.get_form_by_event_id(session, event_id)
            if not form:
                write_log_exception(log_file, Exception(f"HTTP 500: Form for event [{event_id}] not found!!"))
                raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
            write_log(log_file, f"Form found: [{form.id}] of type [{form.form_type}]")
            form_type = form.form_type

            if form_type == "none":
                write_log(log_file, f"Form type is none (Doesn't require registeration), creating member log for member [{member.id}] and log [{event_log.id}]")
...
async def send_certificates(
    ...
    with SessionLocal() as session:
        try:
            ...
            attendance_count = len(attendance)
            write_log(log_file, f"Found [{attendance_count}] attendees who attended all days for event [{event.name}]")
            ...
            cert_request = CertificateRequest(event_name=event.name, announced_name=event.name, date=date_str, official=bool(event.is_official), members=simplified_members)
            write_log_json(log_file, json.dumps({"event_name": event.name, "date": date_str, "official": bool(event.is_official), "member_count": len(simplified_members)}, indent=4, ensure_ascii=False))

            # Make async POST request to certificate API
            certificate_api_url = config.CERTIFICATE_API_URL
            write_log(log_file, f"Sending request to certificate API: [{certificate_api_url}/certificates]")

            async with httpx.AsyncClient(timeout=30.0) as client:
                try:
                    response = await client.post(f"{certificate_api_url}/certificates", json=cert_request.model_dump(mode="json"), headers={"Content-Type": "application/json"})
                    response.raise_for_status()
                    response_data = response.json()
                    write_log(log_file, f"Certificate API responded with job_id: [{response_data.get('job_id')}]")
                    write_log_json(log_file, response_data)
                    return CertificateJobResponse(**response_data)
                except httpx.TimeoutException:
                    write_log_exception(log_file, Exception("Certificate API request timed out after 30 seconds"))
                    raise HTTPException(status_code=status.HTTP_504_GATEWAY_TIMEOUT, detail="Certificate API request timed out")
                except httpx.HTTPStatusError as e:
                    write_log_exception(log_file, Exception(f"Certificate API returned error status {e.response.status_code}: {e.response.text}"))
                    raise HTTPException(status_code=status.HTTP_502_BAD_GATEWAY, detail=f"Certificate API returned error: {e.response.status_code}")
                except httpx.RequestError as e:
                    write_log_exception(log_file, Exception(f"Failed to connect to certificate API: {str(e)}"))
                    raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Failed to connect to certificate API")
        except HTTPException:
            raise
        except Exception as e:
            write_log_exception(log_file, e)
    ...
    with SessionLocal() as session:
        try:
            ...
            for row in submissions_data:
                member = Member_model(id=row.id, name=row.name, email=row.email, phone_number=row.phone_number, uni_id=row.uni_id, gender=row.gender, uni_level=row.uni_level, uni_college=row.uni_college)

                submission = Get_Submission_model(member=member, submission_id=row.submission_id, submitted_at=row.submitted_at, form_type=row.form_type, submission_type=row.submission_type, is_accepted=bool(row.is_accepted), google_submission_value=row.google_submission_value, event_id=row.event_id, form_id=row.form_id, google_form_id=row.google_form_id)

    ...
    with SessionLocal() as session:
        try:
            member = member_queries.get_member_by_uni_id(
                session, get_uni_id_from_credentials(credentials)
            )
            write_log_title(
                log_file,
            ...
            if not event_log:
                write_log_exception(
                    log_file, f"HTTP 500: Event [{event_id}] has no attendable logs!!"
                )
            ...
            if member_logs is None:
                ...
                for member_log in member_logs:
                    ...
                    if (
                        ...
                        raise HTTPException(
                            status_code=status.HTTP_400_BAD_REQUEST,
                            detail="You have already marked attendance for today",
                        )
                    else:
                        write_log(
                            log_file,
            ...
            if not form:
                write_log_exception(
                    log_file, f"HTTP 500: Form for event [{event_id}] not found!!"
                )
                raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
            ...
            if form_type == "none":
                write_log(
                    log_file,
                    f"Form type is none, creating member log for member [{member.id}] and log [{event_log.id}]",
                )
            ...
            if form_type == "google" or form_type == "registration":
                ...
                if not submissions:
                    ...
                    raise HTTPException(
                        status_code=status.HTTP_400_BAD_REQUEST,
                        detail="You have not submitted the form for this event",
                    )
                if submissions.is_accepted == 0:
                    ...
                    raise HTTPException(
                        status_code=status.HTTP_400_BAD_REQUEST,
                        detail="You have not been accepted to the event",
                    )
    ...
    with SessionLocal() as session:
        ...
        try:
            ...
            if new_event is None:
                write_log_exception(
                    log_file,
                    f"HTTP 409: An event with the name '{event_data.event.name}' already exists",
                )
                raise HTTPException(
                    status_code=status.HTTP_409_CONFLICT,
                    detail=f"An event with the name '{event_data.event.name}' already exists",
                )
            ...
            write_log(
                log_file, f"Created Form [{new_form.id}] for Event [{new_event.id}]"
            )
            ...
            days = (
                event_data.event.end_datetime - event_data.event.start_datetime
            ).days + 1
            for day in range(days):
                write_log(
                    log_file, f"Giving department points for day [{day + 1}]/[{days}]"
                )
                log_queries.create_department_log(
                    session, event_data.department_id, department_log.id
                )
...
def update_event(
    ...
    with SessionLocal() as session:
        try:
            ...
            updated_event = events_queries.update_event(
                session, event_id, event_data.event
            )
            ...
            log_queries.update_log_action_id(
                session, department_log.id, department_action.action_id
            )
            write_log(
                log_file,
            ...
            log_queries.update_log_action_id(
                session, member_log.id, member_action.action_id
            )
            write_log(
                log_file,
            ...
            current_dept_id = log_queries.get_department_id_from_log(
                session, department_log.id
            )
            new_dept_id = department_action.department_id
            current_dept_logs_count = log_queries.get_department_logs_count(
                session, department_log.id
            )
            new_days = (
                updated_event.end_datetime - updated_event.start_datetime
            ).days + 1
            ...
            if new_dept_id is not None and current_dept_id != new_dept_id:
                ...
                for day in range(new_days):
                    log_queries.create_department_log(
                        session, new_dept_id, department_log.id
                    )
                write_log(
                    log_file,
                ...
                dept_id_to_use = (
                    new_dept_id if new_dept_id is not None else current_dept_id
                )
                ...
                if new_days > current_dept_logs_count:
                    ...
                    for _ in range(days_to_add):
                        log_queries.create_department_log(
                            session, dept_id_to_use, department_log.id
                        )
                else:
                    ...
                    log_queries.delete_n_department_logs(
                        session, department_log.id, days_to_remove
                    )
...
def add_members_to_event(
    ...
    with SessionLocal() as session:
        ...
        if not log_member:
            excep = HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Event logs not found"
            )
            write_log_exception(log_file, excep)
            raise excep
        write_log(
            log_file, f"Found attendable log [{log_member.id}] for event [{event.id}]"
        )
        ...
        for i, member in enumerate(members):
            ...
            write_log(
                log_file,
                f"Found member [{found_member.name}] with ID [{found_member.id}]",
            )
            log_queries.create_member_log(session, found_member.id, log_member.id, attendance_date)
            write_log(
                log_file, f"Added member [{found_member.name}] to event [{event.name}] for date [{attendance_date}]"
            )
        session.commit()

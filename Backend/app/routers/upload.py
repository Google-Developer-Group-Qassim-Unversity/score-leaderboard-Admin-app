...
def validate_sheet(file_path: str, start_date, end_date):
    ...
    try:
        ...
        raise ValueError({"error": "Could not read the sheet.", "details": str(e)})
    ...
    if sheet_headers != expected_headers:
        ...
        if missing_headers:
            raise ValueError({"error": "Missing columns", "details": missing_headers})
        ...
        for header in extra_headers:
            ...
            if s_header[0:14] != "attendance day" or not s_header[14::].strip().isdigit():
                raise ValueError({"error": "Wrong column.", "details": header})
        ...
        if len(days) > 1:
            if not all(b - a for a, b in zip(days, days[1:])):
                raise ValueError({"error": "Attendace days have to be sequential.", "details": f"Found {days}"})
    ...
    if dups:
        ...
        raise ValueError({"error": "Duplicates in uni id", "details": f"Uni id duplicates on rows {dups}"})
    ...
    if fault_indecies:
        raise ValueError({"error": "Invalid uni ids.", "details": f"uni ids are not numbers on rows: {fault_indecies}"})
    ...
    if fault_length:
        raise ValueError({"error": "Length of uni ids must be 9.", "details": f"Rows: {fault_length}"})
    ...
    if fault_emails:
        raise ValueError({"error": "Invalid emails.", "details": f"Rows: {fault_emails}"})
    ...
    if fault_genders:
        raise ValueError(
            {"error": "Invalid genders.", "details": f"Genders must be either Male or Female on rows: {fault_genders}"}
        )
    return True
...
def bulk_sheet_validation(file_path: str):

...
def get_categorized_actions(session: SessionDep):
...
def get_all_actions(session: SessionDep):
...
def create_action(payload: CreateAction_model, session: SessionDep):
...
def update_action(action_id: int, payload: UpdateAction_model, session: SessionDep):
...
def reorder_actions(payload: ReorderActions_model, session: SessionDep):
...
def delete_action(action_id: int, session: SessionDep, replacement_id: Annotated[Optional[int], Query()] = None):
